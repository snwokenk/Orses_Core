...
class ListenerMessages:
    ...
    """
    # todo: connect reactor instance
    def __init__(self,  messages_heard, netmsginst, msg_type, admin_instance, protocol=None, reactor_inst=None):
        """
...
class ListenerForSendingTokens(ListenerMessages):
    ...
    def __init__(self, messages_heard, netmsginst, msg_type, admin_instance, protocol=None, q_object=None, reactor_inst=None):
        """
            ...
            # usually if wallet pubkey is needed
                ...
                if self.messages_heard[-1] != self.last_msg:  # make sure last msg not b'end' message
                    ...
                    if self.msg_type not in {"misc_msg", "tx_asg"}:  # misc_msg comes with pubkey
                        ...
                        if rsp[0] is True:
                            self.netmsginst.end_convo = True
                            notif_msg = json.dumps(rsp[1]).encode() if not isinstance(rsp[1], (str, bytes)) else \
                                (rsp[1].encode() if isinstance(rsp[1], str) else rsp[1])
                            return notif_msg
                        else:
                            self.netmsginst.end_convo = True
                            return self.reject_msg
